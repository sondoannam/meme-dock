...
export async function getFileMetadata(fileId: string): Promise<FileMetadata> {
  try {
    ...
    if (error instanceof ConfigError || error instanceof FileError) {
      ...
      throw new FileError(error instanceof Error ? `Error fetching file metadata: ${error.message}` : 'Unknown error fetching file metadata');
    }
  }
}
...
export async function listFiles(options: { limit?: number; offset?: number } = {}) {
  try {
    ...
    if (error instanceof ConfigError || error instanceof FileError) {
      ...
      throw new FileError(error instanceof Error ? `Error listing files: ${error.message}` : 'Unknown error listing files');
    }
  }
}
...
export async function deleteFile(fileId: string): Promise<void> {
  try {
    ...
    if (error instanceof ConfigError || error instanceof FileError) {
      ...
      throw new FileError(error instanceof Error ? `Error deleting file: ${error.message}` : 'Unknown error deleting file');
    }
  }
}
